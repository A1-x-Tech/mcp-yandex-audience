...
(`io.github.a1-x-tech/mcp-yandex-audience`). Проверка перед публикацией — все три
...
> членства (`gh api -X PUT /orgs/A1-x-Tech/public_members/<логин>`) необходима, но её одной мало;
> проверить: `curl -s https://api.github.com/users/<логин>/orgs` должен показывать `A1-x-Tech`.
...
2. Указать URL репозитория `https://github.com/A1-x-Tech/mcp-yandex-audience`.
