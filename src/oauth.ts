...
/**
 * A1-x-Tech's OAuth app for Yandex Audience («Яндекс Аудитории — A1-x-Tech MCP»),
 * scoped to segment read/write only. Overridable so a user who wants their own
 * app (own consent screen, own rights) can point the flow at it without forking.
 */
export const DEFAULT_CLIENT_ID = "5bc674cacba54b70a5e50cbc43a6a3e8";
...
/**
 * Requested rights (the Audience API's two scopes: read + create/edit segments,
 * verified against oauth.yandex.ru/client/<id>/info). Sent explicitly even
 * though the app already holds exactly these: without `scope` Yandex issues a
 * token carrying **every** right the app holds, so an app that gains a right
 * later would silently widen every token minted afterwards.
 */
export const SCOPE = "audience:read audience:write";
...
export function oauthClientId(): string {
  return process.env.YANDEX_AUDIENCE_OAUTH_CLIENT_ID || DEFAULT_CLIENT_ID;
}
...
/**
 * No `state` parameter on purpose: state exists to tie a redirect back to the
 * session that started it, and this flow has no redirect — the user copies the
 * code off Yandex's page by hand, and PKCE already binds the exchange to the
  ...
  switch (code) {
    ...
    case "invalid_client":
      return (
        ...
        "Если задан YANDEX_AUDIENCE_OAUTH_CLIENT_ID — проверьте его; " +
