...
export interface AudienceConfig {
  /**
   * Yandex OAuth token from the environment, sent as `Authorization: OAuth …`.
   * Treated as a secret. Absent when YANDEX_AUDIENCE_TOKEN is not set — the
   * server still starts, the TokenStore (auth.ts) falls back to the credentials
   * written by the in-chat login, and a call with no token anywhere raises
   * AuthRequiredError.
   */
  token?: string;
