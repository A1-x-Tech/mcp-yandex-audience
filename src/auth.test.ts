...
async function withTempConfig<T>(run: () => T | Promise<T>): Promise<T> {
  ...
  process.env.XDG_CONFIG_HOME = mkdtempSync(join(tmpdir(), "mcp-audience-test-"));
  try {
    return await run();
  } finally {
...
// The exact not-connected message: it is the only text the calling model reads
// about a missing token, so it is pinned as a literal — a reworded message must
// be a conscious decision, not a drive-by edit. Both fixes must stay named:
// the in-chat login AND the environment variable.
const EXPECTED_NOT_CONNECTED_MESSAGE =
  "Яндекс Аудитории не подключены: нет токена доступа. " +
  ...
  "под аккаунтом Яндекса с доступом к нужным сегментам и прислать код подтверждения, " +
  "затем передайте этот код в finish_login. " +
  "Альтернатива без диалога — задать переменную окружения YANDEX_AUDIENCE_TOKEN " +
  "(OAuth-токен со скоупами чтения и изменения сегментов Аудиторий) " +
  "в конфигурации MCP-клиента и перезапустить сервер.";

test("the exported not-connected message matches the pinned literal", () => {
  assert.equal(NOT_CONNECTED_MESSAGE, EXPECTED_NOT_CONNECTED_MESSAGE);
});
...
test("with nothing configured, getToken explains how to connect", async () => {
  await withTempConfig(async () => {
    ...
    // The message is the user-facing product here: it must name both fixes.
    await assert.rejects(() => store.getToken(), /start_login/);
    await assert.rejects(() => store.getToken(), /YANDEX_AUDIENCE_TOKEN/);
  });
});
...
test("saving credentials leaves no temp files behind", async () => {
  await withTempConfig(() => {
    // Two writes: the second exercises the atomic replace of an existing file.
    writeCredentials({ access_token: "first", obtained_at: 1 });
    const file = writeCredentials({ access_token: "second", obtained_at: 2 });
...
test("a refresh answer without a refresh_token keeps the stored one", async () => {
  await withTempConfig(async () => {
    ...
    // Yandex is allowed to omit refresh_token from a refresh answer; losing the
    // stored one here would make the *next* expiry a forced re-login.
    const fetchImpl = (async () =>
      ({
        ok: true,
        status: 200,
