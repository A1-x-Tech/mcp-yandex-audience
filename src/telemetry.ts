 ...
 * still means "config unusable at load time" (no such checks exist today); it
 * is kept unchanged so the historical funnel stays comparable.
 * `unconfigured_start` is its live counterpart — the server now survives a
 * missing token and completes the handshake, so this is the first event that
