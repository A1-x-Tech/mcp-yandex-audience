 ...
 * it names the fix instead of the failure. It replaces the old CredentialsError
 * (types.ts) — one class, one message, two ways to fix: the in-chat login or
 ...
 *
 *   env    — YANDEX_AUDIENCE_TOKEN, the documented setup; never touched or refreshed
 *   stored — ~/.config/mcp-yandex-audience/credentials.json, written by finish_login
 *
...
export class TokenStore {
  ...
  async getToken(): Promise<string> {
    ...
    if (!stored.refresh_token) {
      throw new AuthRequiredError(
        "Срок действия сохранённого токена Аудиторий истёк, а обновить его нечем. " +
   ...
   * once more when the API answers 401/403 — a token can be revoked in Yandex ID
