...
async function isolated<T>(fn: () => Promise<T>): Promise<T> {
  ...
  process.env.XDG_CONFIG_HOME = mkdtempSync(join(tmpdir(), "mcp-audience-tools-"));
  clearPendingLogin();
  try {
    return await fn();
  } finally {
...
test("start_login returns a PKCE authorize URL for the Audience app, no network", async () => {
  await isolated(async () => {
...
test("finish_login exchanges the code, stores the token and verifies it live", async () => {
  await isolated(async () => {
    ...
    try {
      ...
      assert.equal(res.segmentsVisible, 1);
...
test("finish_login warns when the connected account sees no segments", async () => {
  await isolated(async () => {
    const savedFetch = globalThis.fetch;
    globalThis.fetch = (async () =>
      ({
