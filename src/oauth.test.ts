...
test("the authorize URL asks for a code with S256 and the verification_code redirect", () => {
  ...
  // No `state`: there is no redirect to tie back to a session — the user copies
  // the code by hand, and PKCE already binds the exchange to this process.
...
test("the requested scope is exactly the Audience read/write pair", () => {
  // Pinned: these are the two rights of the «Яндекс Аудитории — A1-x-Tech MCP»
  // app (verified via oauth.yandex.ru/client/<id>/info); a typo here breaks the
  // whole login at the consent screen, after the user already left for the browser.
  assert.equal(SCOPE, "audience:read audience:write");
  assert.equal(DEFAULT_CLIENT_ID, "5bc674cacba54b70a5e50cbc43a6a3e8");
});
...
test("starting a second login discards the first verifier", () => {
  ...
  assert.ok(first && second, "both logins must hold a verifier");
  assert.notEqual(first, second, "the stale verifier must not survive a restart of the flow");
  clearPendingLogin();
});
 ...
 * live endpoint by the Metrica sibling. Handling only the spec's `invalid_grant`
 * sent the most common failure of the whole flow (the code expires in 10 minutes)
 * to the generic branch, which relays "Invalid code" without saying a fresh one
 * is needed.
 */
