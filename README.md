...
Нужны Node.js 20+ и аккаунт Яндекс Аудиторий. OAuth-токен добывать заранее не обязательно: без него сервер сам предложит [вход из диалога](#подключение-из-диалога).

1. Выберите своё AI-приложение и добавьте сервер. Переменная `YANDEX_AUDIENCE_TOKEN` в примерах ниже необязательна — не задавайте её, если планируете войти из диалога ([готовый токен](#получение-доступа-к-api) пригодится для CI и автоматических установок).
2. Попросите ассистента подключить Яндекс Аудитории (или задайте токен в конфигурации).
...
|---|---:|---|---|
| `YANDEX_AUDIENCE_TOKEN` | нет | — | Готовый OAuth-токен — для CI и автоматических установок; необязателен: можно [войти из диалога](#подключение-из-диалога). Имеет приоритет над входом из чата; сервер такой токен не обновляет и не удаляет |
